...
export default function Workouts() {
  ...
  useEffect(() => {
    (async () => {
      try { setData(await fetchWorkouts()); }
      catch (e: any) { Alert.alert("Error", e.message ?? "Failed to load workouts"); }
      finally { setLoading(false); }
    })();
  }, []);

  if (loading) {
    return (<View style={[s.c, s.center]}><ActivityIndicator/><Text style={{marginTop:8}}>Loading workouts…</Text></View>);
  }
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={s.card}>
              ...
              <Pressable style={s.btn}><Text style={s.btnt}>Start</Text></Pressable>
            </View>
          )}
        />
      )}
